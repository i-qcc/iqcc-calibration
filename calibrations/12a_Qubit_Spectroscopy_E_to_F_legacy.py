...
<<<<<<< HEAD:calibrations/12a_Qubit_Spectroscopy_E_to_F.py
# Be sure to include [Parameters, Quam] so the node has proper type hinting
node = QualibrationNode[Parameters, Quam](
    name="12a_Qubit_Spectroscopy_E_to_F",  # Name should be unique
...
import matplotlib.pyplot as plt
import numpy as np
>>>>>>> origin/main:calibrations/12a_Qubit_Spectroscopy_E_to_F_legacy.py
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with program() as node.namespace["qua_program"]:
        # Use machine.declare_qua_variables() for consistency
            ...
            with for_(n, 0, n < n_avg, n + 1):
                ...
                with for_(*from_array(df, dfs)):
                    for i, qubit in multiplexed_qubits.items():
                        ...
                        qubit.xy.play("x180")
                        # Update the qubit frequency to scan around the expected E->F transition
                        # E->F frequency = f_01 + anharmonicity (where anharmonicity is negative)
...
def simulate_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    config = node.machine.get_clean_config()
