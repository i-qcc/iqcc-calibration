...
node = QualibrationNode[Parameters, Quam](name="00a_twpa1_3_calibration", parameters=Parameters())
...
optimized_pump=optimizer(5, 7,  Gain, dsnr,  dsnr_avg, dfps, daps, p_lo,p_if)
for i in range(len(qubits)):
...
if not node.parameters.load_data_id:
    with node.record_state_updates():        
        machine.twpas['twpa1-3'].pump_frequency=dfps[optimized_pump[0]]
        machine.twpas['twpa1-3'].pump_amplitude=daps[optimized_pump[1]]
        machine.twpas['twpa1-3'].max_gain=np.round(np.max(np.mean(Gain,axis=0)))
        machine.twpas['twpa1-3'].max_snr_improvement=np.round(np.max(np.mean(dsnr,axis=0)))
