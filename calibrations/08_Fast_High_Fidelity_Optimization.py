...
class Parameters(NodeParameters):
    ...
    outliers_threshold: float = 0.01 #
    fidelity_threshold_percent: float = 1.0  # Select region within this % of max fidelity
    plot_raw: bool = False
    simulate: bool = False
...
node = QualibrationNode(name="08_Fast_High_Fidelity_Readout_Optimization", parameters=Parameters())
...
if not node.parameters.simulate:
    ...
    def apply_fit_gmm(I, Q):
        """Fit 2-component GMM to I,Q (ground vs excited blobs); return assignment fidelity and inlier fraction.
        GMM = Gaussian Mixture Model: two Gaussians in (I,Q) space for |g> and |e> readout responses.
        'outliers' is the fraction of points with log-likelihood > max_ll + log(0.01) (i.e. well-fit by the model)."""
        ...
        # Fraction of points well-fit by GMM (likelihood within 1% of max); used as quality filter (see valid_amps)
    ...
    best_amp = {}
    best_readout_length = {}
    fidelity_threshold = node.parameters.fidelity_threshold_percent / 100.0
    
    for q in qubits:
        fit_res_q = fit_res.sel(qubit=q.name)
        # valid_amps: amplitudes where at every readout length, the "outliers" value (actually inlier fraction,
        # fraction of I,Q points well-fit by the 2-GMM) is >= outliers_threshold (default 0.98). So we keep only
        # amplitudes with clean, well-separated blobs at all lengths; high amp can fail due to distortion/overlap.
        ...
        # Find max fidelity (only over valid_amps)
        max_fidelity = float(amps_fidelity.max())
        # Define threshold: select points within fidelity_threshold_percent of max
        # e.g., if max is 0.97 and threshold is 1%, select points >= 0.96
        fidelity_cutoff = max_fidelity - fidelity_threshold
        
        # Find all points within threshold region
        valid_region = amps_fidelity.where(amps_fidelity >= fidelity_cutoff, drop=True)
        
        # Among valid points, find the one with shortest readout length
        if valid_region.size > 0:
            # Get minimum readout length in the valid region
            min_readout_length = float(valid_region.readout_length.min())
            # Select points at this minimum readout length
            candidates_at_min_len = valid_region.where(
                valid_region.readout_length == min_readout_length, drop=True
            )
            # Among candidates at min length, pick the one with highest fidelity
            best_loc = candidates_at_min_len.where(
                candidates_at_min_len == candidates_at_min_len.max(), drop=True
            )
            # Handle ties: if multiple points have same fidelity at min length, pick first
            if best_loc.size > 1:
            ...
            # Fallback: use max fidelity point
        ...
        print(f"{q.name}: max_fidelity={max_fidelity:.4f}, threshold={fidelity_cutoff:.4f}")
        ...
        node.results["results"][q.name]["max_fidelity"] = max_fidelity
        node.results["results"][q.name]["fidelity_threshold"] = fidelity_cutoff
        
        # Select data for the best amplitude and readout length
    ...
    for ax, qubit in grid_iter(grid):
        qn = qubit["qubit"]
        fidelity_data = fit_res.loc[qubit].sel(result="meas_fidelity")
        fidelity_data.plot(ax=ax, x="readout_amp", y="readout_length", 
                           robust=True, add_colorbar=True, norm=mcolors.PowerNorm(gamma=3))
        # Draw contour at fidelity threshold (full grid, based on fidelity only)
        fid_threshold = node.results["results"][qn]["fidelity_threshold"]
        try:
            ax.contour(fidelity_data.readout_amp, fidelity_data.readout_length, 
                      fidelity_data.values.T, levels=[fid_threshold], colors='white', linestyles='--', linewidths=1.5)
        except Exception:
            pass  # Skip contour if it fails
        # Red dot = shortest Tro within threshold (among valid_amps)
        ax.scatter(best_amp[qn], best_readout_length[qn], c="red", s=10, zorder=20,
                   label=f"Optimal (shortest Tro within {node.parameters.fidelity_threshold_percent}%)")
        ax.set_xlabel("Readout amplitude (V)")
        ax.set_ylabel("Readout length (ns)")
        max_fid = node.results["results"][qn]["max_fidelity"]*100
        ax.set_title(f"{qn} \n(F: {max_fid:.1f}%, Tro: {best_readout_length[qn]:.0f}ns)")
    grid.fig.suptitle(f"Fast High Fidelity Readout\n shortest Tro within {node.parameters.fidelity_threshold_percent}% of max Fidelity\n{node.date_time} GMT+{node.time_zone} #{node.node_id}\nmultiplexed={node.parameters.multiplexed}, reset={node.parameters.reset_type_thermal_or_active}")
