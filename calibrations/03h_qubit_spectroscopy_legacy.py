...
from iqcc_calibration_tools.quam_config.legacy_tools.save_utils import fetch_results_as_xarray, load_dataset, get_node_id, save_node
...
node = QualibrationNode(name="03h_Qubit_Spectroscopy_legacy", parameters=Parameters())
            ...
            for q in qubits:
                if not np.isnan(result.sel(qubit=q.name).position.values):
                    if flux_point == "arbitrary":
                        ...
                        q.xy.RF_frequency += float(result.sel(qubit=q.name).position.values)
