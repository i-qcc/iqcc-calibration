...
from iqcc_calibration_tools.quam_config.macros import qua_declaration, multiplexed_readout, node_save, active_reset, active_reset_simple
import numpy as np
...
if not node.parameters.simulate:
    ...
    da = ds.flux.sel(qubit = qubit.name).flux

    first_vals = da.isel(time=slice(1, 2)).mean().values
...
if not node.parameters.simulate and not node.parameters.only_FIR: # exponential fit is only done when only_FIR is False
    ...
    filtered_response_long_1exp = lfilter(FIR_1exp,IIR_1exp, da.isel(time=slice(1, None)))

    if plot_process:
        f,ax = plt.subplots()
        ax.plot(da.time,da,label = 'data')
        ax.plot(da.time[1:],filtered_response_long_1exp,label = 'filtered long time 1exp')
...
if not node.parameters.simulate:
    ...
    if node.parameters.only_FIR:
        response_long = da.isel(time=slice(1, None)).values
    else:
    ...
    filtered_response_Full = lfilter(convolved_fir,long_IIR, da.isel(time=slice(1, None)))

    if plot_process:
        da.plot(label =  'data')
        if not node.parameters.only_FIR:
            plt.plot(da.time[1:],filtered_response_long_1exp, label = 'filtered long time')
        plt.plot(da.time[1:],filtered_response_Full, label = 'filtered full, deconvolved')
...
if not node.parameters.simulate:
    ...
    fig,ax = plt.subplots()
    ax.plot(da.time,da/np.mean(da[-10:]),label = 'data')
    if not node.parameters.only_FIR:
        ax.plot(da.time[1:],filtered_response_long_1exp/np.mean(filtered_response_long_1exp[-10:]),'--', label = 'slow rise correction')
    ax.plot(da.time[1:],filtered_response_Full/np.mean(filtered_response_Full[0:10]),'--', label = 'expected corrected response')
    ...
    with node.record_state_updates():
        for qubit in qubits:
            ...
            if qubit.z.opx_output.feedforward_filter is None:
                ...
                fir_list = np.convolve(fir_list_old, fir_est.tolist())[:24]
