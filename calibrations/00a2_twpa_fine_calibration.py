...
print(f"Optimized for qB{poorqubit}")
print(f"@ ap={np.round(optimal_pp,5)},fp={np.round((p_lo+p_if+optimal_fp)*1e-9,3)}GHz ")
multiplexed_qubit_results = {}
for i in range(len(qubits)):        
    qubit_dsnr = np.round(dsnr[i][mtplx_optimized_pump_idx[0]][mtplx_optimized_pump_idx[1]][0], 2)
    qubit_gain = np.round(Gain[i][mtplx_optimized_pump_idx[0]][mtplx_optimized_pump_idx[1]][0], 2)
    print(f"{qubits[i].id}:dSNR:{qubit_dsnr}dB, gain:{qubit_gain}dB")
    multiplexed_qubit_results[qubits[i].id] = {
...
node.results["multiplexed_RO_operation_point"]={'fp':np.round((p_lo+p_if+dfps[mtplx_optimized_pump_idx[0]]),3), 
                 ...
                 'fp_if': np.round(dfps[mtplx_optimized_pump_idx[0]],5),
                 'Pamp': np.round(daps[mtplx_optimized_pump_idx[1]],5)}
node.results["fine_twpa_specs_for_qubits"] = multiplexed_qubit_results
node.results["Ps"]={"Ps":readout_power}
node.results["figures"]={"gain_indiv":gain_indiv,
