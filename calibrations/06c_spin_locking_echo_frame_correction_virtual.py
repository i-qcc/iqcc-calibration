...
# %% {Description}
description = """
        T2 SL MEASUREMENT WITH FRAME ROTATION CORRECTION
The sequence consists in playing an SL sequence (y90 - SL(x,t) - -y90 - measurement) for 
different sl times (spin locking times).

This version includes frame rotation correction using a fixed 5 MHz detuning
to compensate for phase accumulation when switching between xy and xy_sl elements.
...
node = QualibrationNode[Parameters, Quam](
    name="06c_spin_locking_frame_correction",  # Name should be unique
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # Use fixed 5 MHz detuning for frame rotation correction
    # This will be used to compensate for frame rotation when switching between elements
    detuning_hz_fixed = 1e6  # 5 MHz in Hz
    detunings = {}
    for qubit in qubits:
        detunings[qubit.name] = detuning_hz_fixed
            ...
            with for_(shot, 0, shot < n_avg, shot + 1):
                ...
                with for_each_(t_sl, spin_locking_times):
                    ...
                    for i, qubit in multiplexed_qubits.items():
                        ...
                        # Use fixed 5 MHz detuning for phase correction
                        detuning_hz = detunings[qubit.name]  # 5 MHz
                        assign(
                            phase_correction[i],
                            Cast.mul_fixed_by_int(detuning_hz * 1e-9, 128 + 8 * t_sl)
                        )
                        qubit.xy.play("-y90")
                        # Here frame rotation for detuning.
                        ...
                        # Here go back to the frame like in Ramsey
                        qubit.xy.play("-y90")
                    align()
                    # Qubit readout
                    for i, qubit in multiplexed_qubits.items():
