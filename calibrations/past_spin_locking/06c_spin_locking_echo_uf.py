...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    pulse_counts = np.unique(raw_cycles).astype(int) #Register the sweep axes to be added to the dataset when fetching data
        ...
        for multiplexed_qubits in qubits.batch():
            ...
            with for_(shot, 0, shot < n_avg, shot + 1):
                ...
                with for_each_(t_sl, pulse_counts):
                    ...
                    # Qubit manipulation
                    for i, qubit in multiplexed_qubits.items():
                            qubit.xy_sl.update_frequency(5*u.kHz +qubit.xy.intermediate_frequency)
                            qubit.xy.update_frequency(qubit.xy.intermediate_frequency)
                            qubit.xy.play("-y90")
                            qubit.xy_sl.play("x180_BlackmanIntegralPulse_Rise")
                            qubit.xy_sl.play("x180_Square",duration = t_sl)
