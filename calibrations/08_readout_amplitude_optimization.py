    ...
    if "snr_array" in node.results and "readout_amplitude_sweep" in node.results:
        ...
        for i, q in enumerate(qubits):
            ...
            ax.axvline(
                ...
                color="k",
                linestyle="--",
                label=rf"$P_{{\mathrm{{RO}}}}^{{\max}}$ = {p_ro_max_dbm:.2f} dBm",
            )
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    """Update the state file with optimal readout amplitude (and related params) from max-SNR analysis."""
    with node.record_state_updates():
        for q in node.namespace["qubits"]:
            ...
            # Set readout amplitude to the optimal value (max SNR)
