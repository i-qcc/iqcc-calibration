...
class Parameters(NodeParameters):
    ...
    timeout: int = 100  # wait time for job until cancel
    load_data_id: Optional[int] = None
    qubits: Optional[List[str]] = ["qC1"] 
    num_averages: int = 1  # num_shots for the circuit
...
with program() as reset_to_0_exp:
    ...
    machine.initialize_qpu(target=qubits[0])
    # Circuit operations
    for i, qubit in enumerate(qubits):
        # Bring the active qubits to the desired frequency point
        qubit.align()
        qubit.reset(reset_type="active", simulate=node.parameters.simulate)
