...
class Parameters(NodeParameters):
    ...
    readout_amp_scale: float = 1
    readout_time_ns: float = 1500
    timeout: int = 100
    multiplexed: bool = True


node = QualibrationNode(name="readout_propagation", parameters=Parameters())
node_id = get_node_id()
...
# change readout pulse time in state
for qubit in qubits:
    qubit.resonator.operations["readout"].length = node.parameters.readout_time_ns
...
with program() as raw_trace_prog:
    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(n, 0, n < node.parameters.num_averages, n + 1):
            ...
            qubit.resonator.measure("readout", qua_vars=(I[i], Q[i]), stream=adc_st[i], amplitude_scale=node.parameters.readout_amp_scale)
            # save data
            save(I[i], I_st[i])
            save(Q[i], Q_st[i])
    ...
    with stream_processing():
        for i in range(num_qubits):
            # Will save average:
            adc_st[i].input1().real().buffer(node.parameters.num_averages).save(f"adcI{i + 1}")
            adc_st[i].input1().image().buffer(node.parameters.num_averages).save(f"adcQ{i + 1}")
            I_st[i].buffer(node.parameters.num_averages).save(f"I{i + 1}")
            Q_st[i].buffer(node.parameters.num_averages).save(f"Q{i + 1}")
...
if node.parameters.simulate:
    ...
    ds_adc = fetch_results_as_xarray(job.result_handles, qubits, {"time": time_axis, "shot_index": range(node.parameters.num_averages)}, 
    ...
    # %%
    # Fetch the data from the OPX and convert it into a xarray with corresponding axes (from most inner to outer loop)
    ds_IQ = fetch_results_as_xarray(job.result_handles, qubits, {"shot_index": range(node.parameters.num_averages)}, 
                                 keys = ["I1", "Q1", "I2", "Q2"])
    # Add the dataset to the node
