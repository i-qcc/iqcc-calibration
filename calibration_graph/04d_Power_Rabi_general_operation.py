...
from iqcc_research.quam_config.lib.instrument_limits import instrument_limits
...
class Parameters(NodeParameters):
    ...
    operation_x180_or_any_90: Literal["x180", "x90", "-x90", "y90", "-y90", "x180_Gaussian"] = "x180_Gaussian"
...
node = QualibrationNode(name="04b_Power_Rabi_general_operation", parameters=Parameters())
node_id = get_node_id()
...
with program() as power_rabi:
    ...
    with stream_processing():
        n_st.save("n")
        for i, qubit in enumerate(qubits):
            if operation == "x180" or operation == "x180_Gaussian":
                if state_discrimination:
