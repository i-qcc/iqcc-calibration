...
from iqcc_research.quam_config.macros import active_reset, readout_state, readout_state_gef, active_reset_gef
...
from iqcc_research.quam_config.lib.fit import fit_oscillation
...
from iqcc_research.quam_config.components.gates.two_qubit_gates import CZGate, CZWithCompensationGate
from iqcc_research.quam_config.lib.pulses import FluxPulse, SNZPulse
...
class Parameters(NodeParameters):
    ...
    rel_num: int = 25
    spacing: int = 2
    extra_flux_pulse_length: int = 10
    num_frames: int = 10
    load_data_id: Optional[int] = None


node = QualibrationNode(name="31a_SNZ_calibration", parameters=Parameters())
assert not (
                    ...
                    with for_(*from_array(rel_val, np.arange(len(rel_vals)))):
                        ...
                        with for_(*from_array(control_initial, [0, 1])):
                            ...
                            for comp_ind, qubit in enumerate(compensation_qubits):
                                ...
                                qubit.z.wait(qp.qubit_target.xy.operations["x90"].length + 5)
                                qubit.z.play(
                                    "const",
                                    amplitude_scale=shift / qubit.z.operations["const"].amplitude,
                                    duration=qp.gates["Cz_unipolar"].flux_pulse_control.length // 4 + 10,
                                )
...
if not node.parameters.simulate:
    ...
    for q in phase_diff.qubit.values:
        ...
        leak.sel(qubit=q).plot(ax=ax[1], x="amp", vmin=0, vmax=0.2)
